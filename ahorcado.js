...
function verificarFinGanador(){
    ...
    if(palabraOriginalsinLetrasRepetidas===letrasErroneasIngresadas){
        ...
        pincel.fillText("!Ganaste, felicidades! :D",475,165);
        juegoIniciado = false;
        btnReiniciar.focus();
        letrasUnicas = [];
    }
...
function verificarFinPerdedor(){
    if(arrayLetrasIncorrectas.length>5){
        pincel.fillStyle = "red";
        pincel.font = "50px Georgia";
        pincel.fillText("Fin del juego :( intentalo de nuevo!",475,165);
        juegoIniciado = false;
        alert("Perdiste :( la palabra era " + palabraSorteada);
        btnReiniciar.focus();
        letrasUnicas = [];
    }
}
